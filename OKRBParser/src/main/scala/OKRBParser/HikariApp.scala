...
import OKRBParser.infrastructure.endpoints.{AuthEndpoints, OKRBEndpoints, PurchaseEndpoints}
...
import OKRBParser.infrastructure.repository.Postgres.{PostgresAuthRepositoryInterpreter, PostgresOKRBRepositoryInterpreter, PostgresPurchaseRepositoryInterpreter, PostgresUserRepositoryInterpreter}
...
object TestApp extends IOApp {
  ...
  def stream[F[_] : ConcurrentEffect : ContextShift : Timer]: Resource[F, H4Server[F]] = for {
    ...
    endpoints = okrbEndpoints<+>authEndpoints <+> purchaseEndpoints
